...
int main(int argc, char **argv)
{
	//Create a CMT object
	CMT* cmt = new CMT();
	FILE* outputfile = fopen("results/results.txt","w");
	...
	if (challenge_flag)
	{
		...
		video_path = input_path.substr(0, input_path.find("%"));


		//read ground_truth
		read_groundtruth(video_path, &ground_truth);

		//Initialize cmt
		cmt->initialize(im0_gray, ground_truth.at(0));
	...
	while (true)
	{
		...
		Rect pred = cmt->bb_rot.boundingRect();
		int loss = 0;
		double jaccard = getJaccardCoefficient(pred, ground_truth.at(frame));

		//		printf("%.2lf\n", jaccard);
		//		printf("%.2d, %.2d, %.2d, %.2d\n", pred.x, pred.y, pred.width, pred.height);
		//		printf("%.2d, %.2d, %.2d, %.2d\n\n", ground_truth.at(frame).x, ground_truth.at(frame).y,
		//												 ground_truth.at(frame).width, ground_truth.at(frame).height);

		if(jaccard < 0.01)
		{
			*cmt = CMT();
			cmt->initialize(im_gray, ground_truth.at(frame));
			loss = 1;
		...
		fprintf(outputfile, "%.2d,%.2d,%.2d,%.2d,%.2lf,%d\n", pred.x, pred.y, pred.width, pred.height, jaccard, loss);
